...
def draw_detection(frame, det, camera_matrix, dist_coeffs, marker_size,
    ...
    # print("Marker axes expressed in camera frame:")
    # print("X_marker =", rmat[:, 0])
    # print("Y_marker =", rmat[:, 1])
    # print("Z_marker =", rmat[:, 2])

    # print("tvec =", det.tvec.ravel())


    
    roll,pitch,yaw = rotation_to_euler(rmat)

    y = _put(frame, f"id = {det.marker_id}  dist={distance:.1f}m", y, GREEN)
    y = _put(frame, f"x = {x:.2f}  y = {yy:.2f}  z = {z:.2f}", y, GREEN)
