...
class RtspCamera(CameraSource):
    ...
    def _open(self):
        ...
        os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = (
                        "rtsp_transport;tcp|"
                        "threads;1|"
                        "fflags;nobuffer|"          # Bỏ hoàn toàn đệm phân tích stream
                        "flags;low_delay|"          # Ép chế độ giải mã độ trễ cực thấp
                        "max_delay;0|"              # Không chờ đệm đóng gói paket
                        "probesize;32|"             # Giảm dung lượng dò tìm định dạng video
                        "analyzeduration;0"         # Không tốn thời gian phân tích stream đầu vào
                    )
        if not use_gst:
                        # Cấu hình FFMPEG siêu tối ưu cho RTSP Drone (Zero-Latency)
            
            
            cap = cv2.VideoCapture(self.url, cv2.CAP_FFMPEG)
        
        # 3. Đặt bộ đệm OpenCV = 1
            cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
            return cap
        ...
        pipeline = (
            f"rtspsrc location={self.url} protocols={self.protocol} latency={self.latency} drop-on-latency=true ! "
        ...
        print("PINELINENNNNNNN", pipeline)
